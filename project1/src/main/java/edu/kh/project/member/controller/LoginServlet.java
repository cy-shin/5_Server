...
public class LoginServlet extends HttpServlet{
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		try {
			...
			// redirect를 하는 경우 request가 다시 만들어져서 유지되지 않음
//			req.setAttribute("loginMember", loginMember);
			// -> session scope 이용하면 사라지지 않음
			...
			if(loginMember != null) { // 로그인 성공 시
				...
				// 3) 세선 만료 시간 설정(초 단위로 지정)
				//    (클라이언트가 새로운 요청을 할 때 마다, 초기화 된다.)
				 ...
				 *  - 서버로 전달된 쿠키에 값 추가, 수정, 삭제 등을 진행한 후 다시 클라이언트로 반환
				 **/
				
				// 2) 쿠키 객체를 생성
				// - 생성된 쿠키 객체를 resp를 이용해서 클라이언트에게 전달하면
				//   클라이언트 컴퓨터에 파일 형태로 저장됨
				Cookie cookie = new Cookie("saveId", inputEmail); // 입력한 이메일을 쿠키에 저장
				
				// 1) 아이디 저장 체크박스가 체크되었나요
				if(req.getParameter("saveId") != null) { // 체크박스가 체크된 경우
					
					// 3) 쿠키가 유지될 수 있는 유효기간 설정(초 단위)
					cookie.setMaxAge(60 * 60 * 24 * 30);
					
				} else { // 체크되지 않은 경우
					// 4) 체크되지 않은 경우, 쿠키의 유효기간을 0초로 설정함 
					//    = 클라이언트에 저장된 saveId 쿠키를 삭제
				...
				cookie.setPath("/"); // 메인 페이지(/) 주소(http://localhost/)
									 // == 메인 페이지 및 메인 페이지의 하위 주소 모두 적용
					
				// 6) 설정 완료된 쿠키 객체를 클라이언트에 전달
