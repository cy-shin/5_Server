 ...
 * 		
 * 		클라이언트 -> 요청 -> HttpServletRequest/HttpServletResponse 객체 생성 -> [필터] -> Servlet - > 응답
 * 
 * - 여러 필터를 만들어서 연쇄적으로 수행 가능(== FilterChain)
 * 
 * - 만약, 필터 순서를 지정하고자 하면 web.xml 파일에 필터의 이름을 순서대로 작성
 * 
 * */

// @WebFilter annotation : 필터링을 지정할 요청 주소를 작성하는 어노테이션
//							+ 필터로 등록시켜줌
...
public class EncodingFilter extends HttpFilter implements Filter {
	...
	public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain) throws IOException, ServletException {
		...
		// ServletRequest == HttpServletResponse의 부모 인터페이스
