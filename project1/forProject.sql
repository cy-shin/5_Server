...
COMMENT ON COLUMN "MEMBER"."MEMBER_TEL" IS '회원 전화번호(- 없음)';
...
COMMENT ON COLUMN "BOARD"."B_UPDATE_DATE" IS '게시글 마지막 수정일(수정 시 업데이트)';
...
COMMENT ON COLUMN "COMMENT"."COMMENT_DEL_FL" IS '삭제여부(N:삭제 X,  Y:삭제 O)';

COMMENT ON COLUMN "COMMENT"."BOARD_NO" IS '댓글 작성된 게시글 번호';

COMMENT ON COLUMN "COMMENT"."MEMBER_NO" IS '댓글 작성한 회원 번호';

COMMENT ON COLUMN "COMMENT"."PARENT_NO" IS '부모 댓글의 번호';
...
UPDATE BOARD_TYPE SET BOARD_NAME = '질문게시판' WHERE BOARD_CODE = 3;
...
INSERT INTO "MEMBER"
VALUES(SEQ_MEMBER_NO.NEXTVAL, 'user02@kh.or.kr', 'pass02!', '둘리', '01022345678', '04540,,서울시 중구 남대문로 120,,2층',
		DEFAULT, DEFAULT, DEFAULT, DEFAULT);
COMMIT;
-- 아이디, 비밀번호 확인
